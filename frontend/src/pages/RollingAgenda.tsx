...
interface RollingBand {
  ...
  rows: { topic: RollingTopic; cells: (RollingCell | null)[] }[]
}
      ...
      {!columns ? (
        ...
        <Card className="overflow-x-auto">
          <table className="w-full min-w-[720px] text-left text-xs">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-800">
                <th className="sticky left-0 z-10 bg-white py-2 pr-3 align-bottom font-medium text-slate-500 dark:bg-slate-900 dark:text-slate-400">
                  Topic
                </th>
                {meetings.map((meeting) => (
                  <th key={meeting.id} className="min-w-28 px-1 py-2 align-bottom font-medium">
                    <Link to={`/meetings/${meeting.id}`} className="block hover:text-indigo-600 dark:hover:text-indigo-400">
                      ...
                      <span
                        className="block max-w-32 truncate text-[11px] font-normal text-slate-500 dark:text-slate-400"
            ...
            {bands.length === 0 ? (
              <tbody>
                <tr>
                  <td colSpan={columns + 1} className="py-6">
                    <EmptyState
                ...
                <tbody key={band.workstream?.id ?? 'unassigned'}>
                  ...
                  {band.rows.map((row) => (
                    <tr key={row.topic.id} className="border-b border-slate-100 last:border-0 dark:border-slate-800/60">
                      <td className="sticky left-0 z-10 max-w-64 bg-white py-1.5 pr-3 align-top dark:bg-slate-900">
                        <Link
                        ...
                        <span className="mt-0.5 flex flex-wrap items-center gap-1">
                          ...
                          {row.topic.sponsor && (
                            <span className="text-[11px] text-slate-400">{row.topic.sponsor.name}</span>
                          )}
                        </span>
                      </td>
            ...
            {bands.length > 0 && (
              <tfoot>
                <tr className="border-t border-slate-200 dark:border-slate-800">
                  <td className="sticky left-0 z-10 bg-white py-1.5 pr-3 text-[11px] font-medium text-slate-400 dark:bg-slate-900">
                    Total
                  </td>
                  {meetings.map((meeting) => (
          ...
          <p className="mt-2 text-[11px] text-slate-400">
            Cell colour is the topic's intent · a ringed cell is re-timed from its default duration · ✓ means an
            outcome was recorded · click any cell to open that meeting. More than ~10 columns will spill across pages —
            drop the count for a clean print.
          </p>
        </Card>
      )}
    </div>

...
| Area | Capability |
|---|---|
| **Today** | Command dashboard: overdue items, chase queue, decision-ready topics, key dates, today's diary (any linked meeting shown with agenda-capacity bars), plus the pinned person's **discussion list** — the running "things to raise on the next call" — pinned to the top |
...
| **People** | The directory behind every owner field, plus **workstreams** — renamable, recolourable, multi-owner, and drag-orderable, with that one order driving the rolling agenda bands, the planner lanes and every workstream dropdown — plus **People Notes** (feedback, call notes, observations), per-person **discussion lists** (one of which can be pinned to Today), and **1:1 packs** — one click gives you everything a person owns, owes, and hasn't discussed with you yet |
